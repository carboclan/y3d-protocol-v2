<template>
  ...
  <button v-else class="token-select open-currency-select-button" @click="$emit('click')">
    <span class="token-select-symbol">
      ...
      />
      <img v-else
        :src="tokenLogo" class="token-select-symbol-logo" alt="Token logo" :onerror="errImg" />
